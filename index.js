...
console.log(color('Instagram :', 'cyan'), color(`@Kahfz_13`, 'yellow'))
console.log(color('Github :', 'cyan'), color(`@K4HFZ`, 'yellow'))
console.log(color('Wangsap :', 'cyan'), color(`081365814200`, 'yellow'))
	console.log(color('\n', 'cyan'), color('\nJANGAN JUAL SCRIPT INI !!!', 'yellow'))
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./kahfz")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
Semoga Betah Kak ><`
cangtion = `${num.split("@")[0]}, Napa Keluar? 😓?` 
...
Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://www.linkpicture.com/q/20211125_113317.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                    hisoka.send5Loc(anu.id, cangton, ` Name Grup ${metadata.subject} `, Kon, btn)
                } else if (anu.action == 'remove') {
                   hisoka.send5Loc(anu.id, cangtion, `Leaving To  ${metadata.subject} `, Tol, bton)
                }
            }
        } catch (err) {
            console.log(err)
    ...
    hisoka.sendList = async (jid , title = '', text = '', buttext = '', footer = '', but = [], options = {}) =>{
        var template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
        listMessage :{
               title: title,
               description: text,
